...
export function getQuinte(tune: any) {
    let currentIndex = getLevel(tune)
    let nextIndex = currentIndex + 6 + 1
    return getTuneAt(nextIndex);
}

export function getSept(tune: any) {
    let currentIndex = getLevel(tune)
    let nextIndex = currentIndex + 9 + 1
    return getTuneAt(nextIndex);
}

export function getNeuf(tune: any) {
    let currentIndex = getLevel(tune)
    let nextIndex = currentIndex + 9 + 1
    return getTuneAt(nextIndex);
}

export function getTuneAt(nextIndex: number) {
    return GAME[(nextIndex) % GAME.length] + Math.floor(nextIndex / GAME.length);
}

export function getLevel(tune1): number {
...
export function getLevelDegres(tune1): number {
    let base1 = tune1[0]
    let game1 = tune1.slice(-1)
...
export function intervaleDemiTon(tune1, tune2): number {
...
export function intervaleDegres(tune1, tune2): number {
...
export function consonnanceLevel(tune1:string, tune2:string) {
...
export function getSuroundingTunes(tunes: Array<string>) {
...
export function getTunesBeetween(tuneMin: string, tuneMax: string) {
...
export function getNNextDescendante(tune: string = GAME[GAME.length] + 3, n = GAME.length) {
...
export function getNNextAscendente(tune: string = GAME[0] + 3, n = GAME.length) {
    let init = getLevel(tune)
    let tunes = []
    for (let i = 0; i < n; i++)
        tunes.push(getTuneAt(init + i))
