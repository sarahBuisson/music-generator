...
export function fillWithNotesRespecting2(possibilites: Array<string>, rhythme: Array<any>, initTune: string = "", filters: Array<(n: string, n2: ContextType<string>) => boolean> = [], chooseMethode = getRandomFromBestsWhoRespect): Array<any> {
...
export function isStrictlyDescendante(notes: Array<Note>): boolean {
  const levels = notes.map(n => getLevel(n));
  return levels.every((n, index) => index >= levels.length || n > levels[index + 1])
}

export function isStrictlyAscendante(notes: Array<Note>): boolean {
  const levels = notes.map(n => getLevel(n));
  return levels.every((n, index) => index >= levels.length || n < levels[index + 1])
}

export function isMostlyDescendante(notes: Array<Note>): boolean {
  const levels = notes.map(n => getLevel(n));
...
export function isMostlyAscendante(notes: Array<Note>): boolean {
  const levels = notes.map(n => getLevel(n));
  let numberOfDescendante = levels.filter((n, index) => index >= levels.length || n < levels[index + 1]).length;
...
export function computeTuneDerivation(notes: Array<Note>): Array<number> {
  const levels = notes.map(n => getLevel(n));
  let derivation = levels.slice(0, -1).map((n, index) => n - levels[index + 1 % levels.length]);
  return derivation
}

export function numberOfChangeOfMovement(notes: Array<Note>): number {
...
export function fillWithNotesStrictlyDescendantes(_previousNoteTune, rhythme, possibilites, filters: Array<any>, chooseMethode = getRandomFromBestsWhoRespect) {
  return fillWithNotesRespecting(possibilites, rhythme, _previousNoteTune, [isDescendanceOf], chooseMethode);
}


export function insertDisconnanceAndResolutionAtEnd(notes: Array<Note>, possibilites: Array<string>, chooseMethode = getRandomFromBestsWhoRespect) {
...
export function insertResolutionAtEndIfNeed(notes: Array<Note>, possibilites: Array<string>, chooseMethode = getRandomFromBestsWhoRespect) {
  ...
  if (isDissonanteOf(dissonante, last(notes, 0).tune)) {
    ...
    if (!resolution) {
      ...
      let newDissonante = chooseMethode(possibilites, () => 0, [
        isConsonnanteOf, isBeetween(last(notes, 2).tune, last(notes).tune), isDiffOf], {previousTune: last(notes, 0).tune});
      if (newDissonante)
...
export function insertConsonnanteAtSecondeBeginingIfNeeded(notes: Array<Note>, possibilites: Array<string>, chooseMethode = getRandomTuneFromWhoRespect) {

  let isResolutionDescendante = isDescendanceOf(notes[0].tune, notes[1].tune)
  let dissonante = notes[0].tune
  let resolution = chooseMethode(possibilites,
    [isResolutionDescendante ? isDescendanceOf : isAssendanteOf,
      isConsonnanteOf],
    {previousTune: dissonante})
...
export function flatPartition(form: Array<any>): Array<Note> {
...
export function mapMultiLevel(array: Array<any>, callback): Array<any> {
...
export function fillWithNotesRespecting(possibilites: Array<string>, rhythme: Array<any>, initTune: string = "", filters: Array<(n: string, n2: ContextType<string>) => boolean> = [], chooseMethode = getRandomFromBestsWhoRespect
): Array<any> {
  let previousTune = initTune;
  const notes = mapMultiLevel(rhythme, (value) => {
...
export function derivationHarmonique(notes: Array<string>): Array<number> {
...
export function  derivationRythme(notes: Array<string>): Array<number> {
...
export function extremumHarmonique(notes: Array<string>) {
...
export function extremumRhyhtme(notes: Array<string>) {
