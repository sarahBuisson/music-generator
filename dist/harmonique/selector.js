import { consonnanceLevel, getLevel } from "../harmoniqueUtils";
...
export function isAssendanteOf(newTune, context) {
    return getLevel(context.previousTune) < getLevel(newTune);
}
export function isDescendanceOf(newTune, context) {
    return getLevel(context.previousTune) > getLevel(newTune);
}
export function isSameSens(noteA, noteB) {
    if (isAssendanteOf(noteA, { previousTune: noteB }))
        ...
        return (tune2, context) => context.previousTune === tune2;
}
...
export function notIn(forbiddenValues) {
    return (newTune, context) => {
